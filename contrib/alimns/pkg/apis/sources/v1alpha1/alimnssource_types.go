...
// Check that AliMnsSource can be validated and can be defaulted.
var _ runtime.Object = (*AliMnsSource)(nil)

// Check that AliMnsSource implements the Conditions duck type.
var _ = duck.VerifyType(&AliMnsSource{}, &duckv1alpha1.Conditions{})

const (
	// MNSSourceEventType is the MNS CloudEvent type, in case MNS doesn't send a
	// CloudEvent itself.
	AliMnsSourceEventType = "alicloud.mns.topic"
)

// AliMnsEventSource returns the MNS CloudEvent source value.
func AliMnsEventSource(topic string) string {
	return fmt.Sprintf("%s", topic)
}

// AliMnsSourceSpec defines the desired state of AliMnsSource
type AliMnsSourceSpec struct {
...
// AliMnsSourceStatus defines the observed state of AliMnsSource
type AliMnsSourceStatus struct {
...
// AliMnsSource is the Schema for the mnssources API
// +k8s:openapi-gen=true
type AliMnsSource struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   AliMnsSourceSpec   `json:"spec,omitempty"`
	Status AliMnsSourceStatus `json:"status,omitempty"`
}
...
func (s *AliMnsSourceStatus) GetCondition(t duckv1alpha1.ConditionType) *duckv1alpha1.Condition {
	return mnsSourceCondSet.Manage(s).GetCondition(t)
}

// IsReady returns true if the resource is ready overall.
func (s *AliMnsSourceStatus) IsReady() bool {
	return mnsSourceCondSet.Manage(s).IsHappy()
}

// InitializeConditions sets relevant unset conditions to Unknown state.
func (s *AliMnsSourceStatus) InitializeConditions() {
	mnsSourceCondSet.Manage(s).InitializeConditions()
}

// MarkSink sets the condition that the source has a sink configured.
func (s *AliMnsSourceStatus) MarkSink(uri string) {
	s.SinkURI = uri
	if len(uri) > 0 {
...
func (s *AliMnsSourceStatus) MarkTransformer(uri string) {
...
func (s *AliMnsSourceStatus) MarkNoSink(reason, messageFormat string, messageA ...interface{}) {
...
func (s *AliMnsSourceStatus) MarkEventTypes() {
...
func (s *AliMnsSourceStatus) MarkNoEventTypes(reason, messageFormat string, messageA ...interface{}) {
	mnsSourceCondSet.Manage(s).MarkFalse(MNSConditionEventTypesProvided, reason, messageFormat, messageA...)
}

// AliMnsSourceList contains a list of AliMnsSource
type AliMnsSourceList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []AliMnsSource `json:"items"`
}

func init() {
	SchemeBuilder.Register(&AliMnsSource{}, &AliMnsSourceList{})
}
