...
import (
	...
	"github.com/knative/eventing-contrib/contrib/alitablestore/pkg/reconciler/resources"
...
const (
	...
	controllerAgentName = "alitablestore-source-controller"
...
// Add creates a new AliTablestoreSource Controller and adds it to the Manager with default RBAC. The Manager will set fields on the Controller
...
func Add(mgr manager.Manager, logger *zap.SugaredLogger) error {
	...
	log.Println("Adding the Tablestore Source controller.")
	p := &sdk.Provider{
		AgentName: controllerAgentName,
		Parent:    &v1alpha1.AliTablestoreSource{},
...
func (r *reconciler) Reconcile(ctx context.Context, object runtime.Object) error {
	...
	src, ok := object.(*v1alpha1.AliTablestoreSource)
	if !ok {
		logger.Error("could not find Tablestore source", zap.Any("object", object))
		return nil
	}
	...
	//     - Will be garbage collected by K8s when this AliTablestoreSource is deleted.
	...
	// finalizer to every AliTablestoreSource.
...
func (r *reconciler) addFinalizer(s *v1alpha1.AliTablestoreSource) {
	finalizers := sets.NewString(s.Finalizers...)
	finalizers.Insert(finalizerName)
	s.Finalizers = finalizers.List()
}

func (r *reconciler) removeFinalizer(s *v1alpha1.AliTablestoreSource) {
	finalizers := sets.NewString(s.Finalizers...)
	finalizers.Delete(finalizerName)
	s.Finalizers = finalizers.List()
}

func (r *reconciler) createReceiveAdapter(ctx context.Context, src *v1alpha1.AliTablestoreSource, subscriptionID, sinkURI, transformerURI string) (*appsv1.Deployment, error) {
...
func (r *reconciler) getReceiveAdapter(ctx context.Context, src *v1alpha1.AliTablestoreSource) (*appsv1.Deployment, error) {
...
func (r *reconciler) getLabelSelector(src *v1alpha1.AliTablestoreSource) labels.Selector {
	return labels.SelectorFromSet(getLabels(src))
}

func getLabels(src *v1alpha1.AliTablestoreSource) map[string]string {
...
func (r *reconciler) createSubscription(ctx context.Context, src *v1alpha1.AliTablestoreSource) (string, error) {
	creds, err := GetCredentials(ctx, r.client, src)
	if err != nil {
		logging.FromContext(ctx).Desugar().Info("Failed to extract creds", zap.Error(err))
		return "", err
	}
	psc, err := r.tunnelClientCreator(ctx, src.Spec.Instance, creds)
	if err != nil {
		return "", err
	}
...
func (r *reconciler) deleteSubscription(ctx context.Context, src *v1alpha1.AliTablestoreSource) error {
	creds, err := GetCredentials(ctx, r.client, src)
	if err != nil {
		logging.FromContext(ctx).Desugar().Info("Failed to extract creds", zap.Error(err))
		return err
	...
	if exists, err := psc.SubscriptionExists(ctx, src.Spec.TableName, generateSubName(src)); err != nil {
		return err
	} else if !exists {
		return nil
	}
	return psc.DeleteSubscription(ctx, src.Spec.TableName, generateSubName(src))
}

func (r *reconciler) reconcileEventTypes(ctx context.Context, src *v1alpha1.AliTablestoreSource) error {
...
func generateSubName(src *v1alpha1.AliTablestoreSource) string {
